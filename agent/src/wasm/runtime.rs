...
impl WasmRuntime {
    ...
    pub fn new(filter_path: &Path) -> Result<Self> {
        ...
        let module = Module::from_file(&engine, filter_path)
    ...
    pub fn execute(&mut self, input: &FilterInput) -> Result<FilterResult> {
        ...
        let input_bytes = bincode::serialize(input)
            .context("Failed to serialize filter input")?;
        ...
        // Deserialize output
        let result: FilterResult = bincode::deserialize(&output_bytes)
            .context("Failed to deserialize filter output")?;
