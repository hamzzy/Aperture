            ...
            match event {
                ...
                ProfileEvent::Lock(ev) => {
                    ...
                    if !ev.stack_trace.is_empty() {
                        let stack = Stack::from_ips(&ev.stack_trace);
...
mod tests {
    ...
    fn test_aggregate_cpu_samples() {
        let payload = make_payload(vec![
            ...
            ProfileEvent::CpuSample(CpuSample {
                ...
                tid: 1,
                cpu_id: 0,
                user_stack: vec![0x1000, 0x2000], // same stack
                kernel_stack: vec![],
                comm: "test".to_string(),
            }),
            ProfileEvent::CpuSample(CpuSample {
        ...
        assert_eq!(cpu.samples.len(), 2); // 2 unique stacks
    ...
    fn test_aggregate_multiple_batches() {
        let p1 = make_payload(vec![ProfileEvent::CpuSample(CpuSample {
            timestamp: 1000,
            pid: 1,
            tid: 1,
            cpu_id: 0,
        ...
        let p2 = make_payload(vec![ProfileEvent::CpuSample(CpuSample {
            timestamp: 2000,
            pid: 1,
            tid: 1,
            cpu_id: 0,
            user_stack: vec![0x1000], // same stack
            kernel_stack: vec![],
            comm: "test".to_string(),
        })]);
        ...
        assert_eq!(cpu.samples.len(), 1); // same stack merged
    }
