...
### CPU Profiler (`agent-ebpf/src/cpu_profiler.rs`)
...
### Lock Profiler (`agent-ebpf/src/lock_profiler.rs`)
...
### Syscall Tracer (`agent-ebpf/src/syscall_tracer.rs`)
...
Two code paths exist for resolving instruction pointer (IP) addresses to function names:

1. **Local path** (`SymbolResolver`): Used when generating local output (flamegraph.svg, JSON). Splits IPs by address range:
   - `>= 0xffff_0000_0000_0000` → kernel (resolves via `/proc/kallsyms`)
   - Others → userspace (resolves via `/proc/PID/maps` + debug symbols)

2. **Aggregator push path** (`SymbolCache`): Pre-resolves symbols before sending to aggregator. Same kernel/user split. Encodes module info: `"function_name [module_basename]"`.
