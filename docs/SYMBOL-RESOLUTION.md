...
When functions appear as raw hex addresses (e.g. `0xffff8b5b`) in the flamegraph or top functions table, it means the agent could not resolve the instruction pointer to a function name. This guide covers common causes and fixes.
...
**Symptom**: User-space function addresses are unresolved.
...
If the kernel resolution shows `0/N`, check `kptr_restrict`. If user resolution shows `0/N`, check that the target process is running and has debug info.
